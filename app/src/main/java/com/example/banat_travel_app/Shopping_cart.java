...
public class Shopping_cart extends AppCompatActivity // clasa in care se descrie functionalitatea cosului de cumparaturi
{
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        FirebaseRecyclerAdapter<Activity_details, Activity_details_viewHolder> recyclerAdapter=new FirebaseRecyclerAdapter<Activity_details, Activity_details_viewHolder>(Activity_details.class, R.layout.shopping_cart_card_design, Activity_details_viewHolder.class, myRef)
        { // viewholder custom luat de pe youtube
            @Override
    ...
    {
        myRef.keepSynced(true); // facem ca modificarile operate in baza de date Firebase sa fie in timp real
    ...
    {
        checkout.setOnClickListener(new View.OnClickListener() // daca dam click pe checkout
        {
            @Override
            public void onClick(View v)
            {
                if(resultFrom!=null && !resultFrom.isEmpty() && resultTo!=null && !resultTo.isEmpty()) // putem face checkout doar daca rezervarea are datele de inceput si de final setate
                {
                    myRef.removeValue().addOnCompleteListener(new OnCompleteListener<Void>() // stergem toate rezervarile din cos si afisam un mesaj daca checkout-ul are succes
                    {
                        @Override
                        public void onComplete(@NonNull Task<Void> task)
                        {
                            if(task.isSuccessful())
                                Toast.makeText(Shopping_cart.this, "We wish you a wonderful holiday!", Toast.LENGTH_SHORT).show();
                        }
                    });
                }
                else Toast.makeText(Shopping_cart.this, "Please make sure that there are any reservations or if the 'from' and 'to' dates are set!", Toast.LENGTH_LONG).show(); // daca nu sunt setate datele, nu putem face checkout
            }
        });
        ...
        logout.setOnClickListener(new View.OnClickListener() // daca dam click pe log out
        {
            ...
            {
                fbAuth.signOut();
                Intent intent=new Intent(Shopping_cart.this, Login.class);
                finishAffinity();
    ...
    @Override
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) // aici verificam daca am primit cele doua date din activitatea 'Shopping_cart_details', pentru a putea trece la checkout
    {
    ...
    public static class Activity_details_viewHolder extends RecyclerView.ViewHolder // clasa customizata pentru viewholder (luata de pe youtube) -> fiecare rezervare din baza de date este afisata sub forma unui cardview ce contine un titlu (numele rezervarii) si pretul, precum si numarul de persoane
    {
        View view;
